...
function criarPlaylist() {
    const nome = prompt("Qual o nome da tua nova Playlist?");
    if (nome) {
...
function carregarPlaylists() {
    ...
    lista.innerHTML = playlists.map(nome => `
        <div class="user-playlist-item" onclick="search('${nome}', 'playlist')">
            <i class="fa-solid fa-music" style="font-size: 10px;"></i> ${nome}
        </div>
    `).join('');
}
